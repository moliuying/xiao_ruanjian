            ...
            <div class="innerConnet">
                <div class="innerConnetBox">
                    ...
                    <div class="step2" v-if="step == 2">
                        <ul>
                            <li @click="chuti('简单')">简单</li>
                            <li @click="chuti('正常')">正常</li>
                            <li @click="chuti('困难')">困难</li>
                        </ul>
    ...
    export default {
        ...
        methods: {
            ...
            setData(title,allData){
                ...
                console.log( path.join(homedir, year+':'+mounth+':'+day+' '+HH+':'+mm+':'+ss+'_'+title+`导出题目.docx`),' path.join(homedir, year+\':\'+mounth+\':\'+day+\' \'+HH+\':\'+mm+\':\'+ss+\'_\'+title+`导出题目.docx`)')
                // fs.writeFileSync(   path.join(process.cwd(), `设备清单`, year+':'+mounth+':'+day+' '+HH+':'+mm+':'+ss+'_'+title+`导出题目.docx`), buf);
                fs.writeFileSync(  path.join(homedir, year+':'+mounth+':'+day+' '+HH+':'+mm+':'+ss+'_'+title+`导出题目.docx`), buf);


                ElMessage({type: 'success', message: title + '试卷导出成功'})
            },
            getTenNum(n,arr) {
                var result = [];
